...
function renderStatus() {
  ...
  renderDeploy(st);
  if (st.mock) setBadge("error", "依赖缺失");
  else if (!st.initialized) setBadge("warn", "未初始化");
  else if (st.interface_up) setBadge("ok", "运行中");
  else setBadge("warn", "接口未运行");
  const s = st.settings || {};
...
async function deployContainer() {
  ...
  if (!body.config_dir) { toast("请填写配置目录映射路径", "err"); return; }
  const btn = $("deployBtn");
  btn.disabled = true;
  btn.textContent = "正在部署 Docker 容器（拉取镜像可能需要几分钟）...";
  try {
    const res = await api("/api/container/deploy", { method: "POST", body: JSON.stringify(body) });
    toast(res.message + "（配置目录：" + res.config_dir + "）", "ok");
    await loadStatus();
    await loadPeers();
  } catch (e) { toast(e.message, "err"); }
  finally { btn.disabled = false; btn.textContent = "部署 WireGuard 容器"; }
}
...
function peerCard(p) {
  ...
  <div class="peer-card">
    ...
    <div class="peer-info">
      <div class="peer-name"><span>${esc(p.name)}</span><span class="route-tag" title="客户端路由模式">${route}</span><span class="peer-state ${online ? "s-on" : "s-off"}"><span class="sdot"></span>${online ? "在线" : "离线"}</span></div>
      <div class="peer-meta"><span>内网 IP <b>${esc(p.ip)}</b></span><span>握手 <b>${esc(p.handshake)}</b></span><span>↓ <b>${rx}</b></span><span>↑ <b>${tx}</b></span></div>
    </div>
    <div class="peer-actions">
      <button class="btn btn-act" data-act="qr" data-id="${p.id}" title="二维码 / 配置" aria-label="二维码"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><path d="M14 14h3v3h-3zM20 14h1M14 20h1M18 18h3"/></svg></button>
      <button class="btn btn-act" data-act="dl" data-id="${p.id}" title="下载配置" aria-label="下载"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="M7 10l5 5 5-5"/><path d="M12 15V3"/></svg></button>
      <button class="btn btn-act btn-del" data-act="del" data-id="${p.id}" title="删除客户端" aria-label="删除"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M10 11v6M14 11v6"/></svg></button>
    </div>
  </div>`;
}
...
function openCreate() {
  if (!state.initDone) { toast("请先完成服务器初始化", "err"); return; }
  $("peerName").value = "";
  $("peerCount").value = "1";
  $("routeCidr").value = "";
  setRouteMode("full");
...
async function confirmCreate() {
  ...
  if (!name) { toast("请输入客户端名称 / 前缀", "err"); return; }
  if (count < 1 || count > 50) { toast("创建数量必须在 1-50 之间", "err"); return; }
  if (mode === "split" && !$("routeCidr").value.trim()) { toast("分流模式请填写客户端路由（真实内网网段）", "err"); return; }
  const btn = $("createConfirm");
  btn.disabled = true;
  try {
    if (count === 1) {
      const res = await api("/api/peers", { method: "POST", body: JSON.stringify({ name, route_mode: mode, route_cidr: $("routeCidr").value.trim() }) });
      ...
      const res = await api("/api/peers/batch", { method: "POST", body: JSON.stringify({ count, name_prefix: name, route_mode: mode, route_cidr: $("routeCidr").value.trim() }) });
      $("createModal").hidden = true;
...
async function openDetail(peer, config) {
  if (!config) {
    try {
      ...
      config = await r.text();
    } catch (e) { toast("获取配置失败", "err"); return; }
  }
  ...
  try {
    ...
    box.appendChild(img);
  } catch (e) { box.innerHTML = '<p class="hint">二维码生成失败，可直接下载配置文件</p>'; }
...
function copyConfig() {
  ...
  } else { fallbackCopy(state.currentConfig, done); }
}
...
function copyKey() {
  ...
  if (!key || key === "尚未生成") { toast("服务器公钥尚未生成", "err"); return; }
...
function bindEvents() {
  ...
  $("refreshBtn").addEventListener("click", () => { loadStatus(); toast("已刷新", "ok"); });
  ...
  $("peerName").addEventListener("keydown", (e) => { if (e.key === "Enter") confirmCreate(); });
  document.querySelectorAll("#routeSeg .seg-btn").forEach((b) => { b.addEventListener("click", () => setRouteMode(b.dataset.route)); });
  $("peerList").addEventListener("click", (e) => {
    ...
    if (act === "qr") openDetail(p);
      ...
      fetch("/api/peers/" + id + "/config")
        .then((r) => r.text())
        .then((cfg) => { state.currentConfig = cfg; downloadConfig(); })
        .catch(() => toast("下载失败", "err"));
    } else if (act === "del") deletePeer(id);
  });
  document.querySelectorAll("[data-close]").forEach((b) => {
    b.addEventListener("click", () => { const m = b.closest(".modal-mask"); if (m) m.hidden = true; });
  });
...
loadStatus().then(() => {
  setInterval(() => {
    if (document.visibilityState === "visible") loadStatus(true).then(() => loadPeers());
  }, 10000);
});
