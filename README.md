...
| 服务器配置 | 公网 IP/域名（Endpoint）、UDP 端口（默认 51820）、内网网段（默认 `10.13.13.0/24`）、DNS、KeepAlive、MTU、NAT 转发，公钥一键复制 |
| 客户端管理 | 一键创建、批量生成（自动编号）；全量代理或仅内网分流（AllowedIPs）；二维码扫码导入；配置下载/复制；重命名、启用/停用；搜索过滤；删除即时生效 |
| 密码认证 | SHA-256 加盐哈希 + Cookie 会话令牌；可设置/修改密码、随时关闭认证；未登录时自动拦截 API 请求 |
| 配置备份 | 一键导出完整配置为 JSON 文件；从 JSON 导入恢复或迁移（保留当前密码认证设置） |
...
1. 下载 `dist/fn-wg-web_0.3.0_all.fpk`
...
客户端卡片支持以下操作：二维码/配置查看、配置下载、重命名、启用/停用、删除。停用的客户端从 wg0.conf 移除并重启容器生效，配置保留可随时重新启用。列表上方搜索框支持按名称或 IP 实时过滤。

### 密码认证

在「安全与备份」区块点击「设置密码」可启用密码认证。启用后访问管理面板需输入密码登录（浏览器 Cookie 保持会话）。可随时修改密码或关闭认证。建议在内网之外访问时开启。

### 配置备份

在「安全与备份」区块点击「导出配置」下载完整配置 JSON 文件（含服务器密钥与全部客户端）。点击「导入配置」选择 JSON 文件可恢复或迁移配置（保留当前密码认证设置，导入后自动应用到运行时）。
